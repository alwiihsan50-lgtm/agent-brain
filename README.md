...
- **Last Updated At:** 2026-08-14 04:02 WIB
...
- [x] Implementasi dan pengujian live **Automasi Trading MetaTrader 5 (Docker + Exness + Wine Python)** yang terhubung langsung ke **Apple Safari iOS Web Push Notification** via **Cloudflare Tunnel (`cloudflared`)** permanen dengan custom domain, serta Web UI Dokumentasi lokal di `docs/mt5-docker-forex-trading-automation.md`.
...
2. **Apple Safari Web Push Notification & Cloudflare Tunnel:** Backend Node.js Express (`/home/cuker/bot_push_server`) di port `3005` menangani web-push VAPID dan subscription iPhone. Dihubungkan secara permanen ke internet publik menggunakan Cloudflare Tunnel (`cloudflared tunnel run --url http://localhost:3005 mt5-push`) pada custom domain pengguna. Notifikasi saldo & status trading bot berhasil terkirim dan diverifikasi live di iPhone.
...
9. **Lingkungan & Reserved Ports:** Port **`53317`** (TailShare), Port **`3000`** (MT5 Web VNC GUI), Port **`3005`** (Bot Push Notification Server).

---

## 📚 Indeks Dokumentasi (`docs/`)
- 📄 [Panduan Automasi Trading MT5 & Push Notification iOS](docs/mt5-docker-forex-trading-automation.md) - Arsitektur Docker MT5, Wine Python MetaTrader5, backend Push Notification port 3005, Cloudflare Tunnel, dan troubleshooting.
...
- 📄 [System Environment & Reserved Ports](docs/system-environment-and-ports.md) - Informasi spesifikasi sistem Windows/Linux, konfigurasi GitHub CLI, dan aturan Port terpesan (53317, 3000, 3005).
